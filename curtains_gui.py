...
class ProcessTable(ft.DataTable):
    def __init__(self, ):
        ...
        self.columns = [
            ...
            ft.DataColumn(ft.Text('locked'), on_sort=self.sort_rows)
        ]
    ...
    def build_new_row(self, icon, pid, pname, wcount, hidden):
        if not isinstance(icon, str):
            icon_cell = ft.DataCell(
                ft.CircleAvatar(content=ft.Text(pname[0:2].title(), size=8, weight=ft.FontWeight.BOLD), radius=8,
                                bgcolor=ft.colors.ERROR))
        else:
            icon_cell = ft.DataCell(ft.Image(src_base64=icon, width=16, height=16,
...
class Navbar(ft.AppBar):
    def __init__(self, p_table):
        ...
        self.title = ft.Row(spacing=13,
                            controls=[ft.Icon(ft.icons.DESKTOP_ACCESS_DISABLED, color=ft.colors.INVERSE_PRIMARY),
                                      self.searchbox])
        self.leading_width = 40
        # title=ft.Text("Curtains", italic=True, color=ft.colors.INVERSE_PRIMARY, size=14),
